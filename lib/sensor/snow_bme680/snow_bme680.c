...
static struct bme680_dev       bme_device;
static nrf_drv_twi_t*          m_twi;
static int16_t                 m_meas_period;


int8_t snow_bme680_init(nrf_drv_twi_t* twi, int8_t amb_temp) {
    m_twi = twi;
...
int8_t snow_bme680_conf() {
    ...
    if (result == BME680_OK) {
        bme680_get_profile_dur(&m_meas_period, &bme_device);
    }
